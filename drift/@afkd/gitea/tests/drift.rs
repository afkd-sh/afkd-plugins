//! The gate that holds the **shipped `@afkd/gitea` plugin** — the `gitea` and
//! `gitea_pr_review` trigger kinds, built from source at install time — to a real afkd: the
...
//!   renames both kinds (`gitea_probe`, `gitea_pr_review_probe`) and whose `exec` is a
...
fn install(home: &Path, root: &Path) -> Install {
    ...
    let refused = ["gitea", "gitea_pr_review"].iter().any(|kind| {
...
/// The PR rule runs first, so `gitea_probe` can never match inside `gitea_pr_review_probe`.
...
{
  IFS= read -r hello
  printf '%s\n' "$hello" | sed -e 's/"kind":"gitea_pr_review_probe"/"kind":"gitea_pr_review"/' \
                               -e 's/"kind":"gitea_probe"/"kind":"gitea"/'
  cat
...
fn probe_manifest(staged: &Path) {
    ...
    let rewrites = [
        (r#"kind = "gitea""#, r#"kind = "gitea_probe""#),
        (
            r#"kind = "gitea_pr_review""#,
            r#"kind = "gitea_pr_review_probe""#,
        ),
        (
...
fn install_leg_is_refused_while_the_kind_is_built_in_and_runs_once_it_is_not() {
    ...
    match install(home.path(), &staged) {
        Install::BuiltIn(report) => eprintln!(
            "skipping: the afkd on PATH still has `gitea` built in, so it refuses {NAME} \
        ...
        Install::Placed => {
            ...
            drive_one_issue(home.path(), "gitea");
        }
    }
}
...
fn the_plugin_binary_runs_on_the_real_spine_under_probe_kinds() {
    ...
    drive_one_issue(home.path(), "gitea_probe");
}
...
fn every_readme_conf_fence_validates() {
    ...
    match install(home.path(), &stage(stage_dir.path())) {
        Install::BuiltIn(_) => eprintln!(
            "the afkd on PATH has `gitea` built in: the fences validate against its vocabulary"
        ),
...
fn the_manifest_transcribes_the_in_tree_tables() {
    ...
    let afkd_settings = "crates/gitea/src/settings.rs";
    let afkd_lifecycle = "crates/forge/src/lifecycle.rs";
    let afkd_lib = "crates/gitea/src/lib.rs";
    let (settings, lifecycle, lib) = (
        read(&src.join(afkd_settings)),
        read(&src.join(afkd_lifecycle)),
        read(&src.join(afkd_lib)),
    );
    let ported_settings = "@afkd/gitea/src/settings.rs";
    let ported_lifecycle = "@afkd/gitea/src/lifecycle.rs";
