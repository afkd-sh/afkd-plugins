...
//!
//! The Trello the plugin talks to is [`fake`], the stateful loopback fake the plugin's own
//! wire suite runs against — included from the plugin's tree, never copied, so the two
//! suites cannot come to disagree about what Trello does.
//!
//! ## No hangs on failure
//!
...
#[path = "../../../../../@afkd/trello/tests/common/fake.rs"]
pub mod fake;
...
/// from the `target/release/afkd-trello` its `exec` names. `RUSTUP_HOME` and `CARGO_HOME`
