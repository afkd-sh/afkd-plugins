//! The gate that holds the **shipped `@afkd/github` plugin** — the `github` and
//! `github_pr_review` trigger kinds, built from source at install time — to a real afkd: the
...
//!   renames both kinds (`github_probe`, `github_pr_review_probe`) and whose `exec` is a
...
//! wait is bounded and every spawn held in a [`Daemon`]: a claim that never lands must
//! **fail**, not hang.
...
fn plugin_root() -> PathBuf {
    let root = concat!(env!("CARGO_MANIFEST_DIR"), "/../../../@afkd/github");
...
fn install(home: &Path, root: &Path) -> Install {
    ...
    let refused = ["github", "github_pr_review"].iter().any(|kind| {
...
/// The PR rule runs first, so `github_probe` can never match inside `github_pr_review_probe`.
...
{
  IFS= read -r hello
  printf '%s\n' "$hello" | sed -e 's/"kind":"github_pr_review_probe"/"kind":"github_pr_review"/' \
                               -e 's/"kind":"github_probe"/"kind":"github"/'
  cat
} | "$here/target/release/afkd-github"
"#;
...
fn probe_manifest(staged: &Path) {
    ...
    let rewrites = [
        (r#"kind = "github""#, r#"kind = "github_probe""#),
        (
            r#"kind = "github_pr_review""#,
            r#"kind = "github_pr_review_probe""#,
        ),
        (
            r#"exec = "target/release/afkd-github""#,
            r#"exec = "probe-exec""#,
        ),
    ];
...
/// The repository, as the wire suite names it: the fake keys its state by this string.
...
fn service(home: &Path, kind: &str, host: &str) -> String {
    format!(
        r#"service widgets {{
  work_dir "{home}"
  trigger {kind} {{
...
/// Everything a failed wait needs to say: the issue as the fake holds it, its comments, the
/// run dirs and the daemon's own log.
fn dump(home: &Path, fake: &FakeGithub) -> String {
...
fn wait_for(home: &Path, fake: &FakeGithub, what: &str, ready: impl Fn() -> bool) {
...
/// Whether `fake`'s comments on [`ISSUE`] hold a claim marker.
...
fn drive_one_issue(home: &Path, kind: &str) {
    ...
    write_config(home, &service(home, kind, fake.host()));
    ...
    wait_for(home, &fake, "the issue is claimed", || {
        let labels = fake.issue_state(REPO, ISSUE).labels;
        marked(&fake)
...
fn install_leg_is_refused_while_the_kind_is_built_in_and_runs_once_it_is_not() {
    ...
    match install(home.path(), &staged) {
        Install::BuiltIn(report) => eprintln!(
            "skipping: the afkd on PATH still has `github` built in, so it refuses {NAME} \
             ({report}); this leg goes live once afkd drops the built-in"
        ),
        Install::Placed => {
            let exec = plugins_root(home.path())
                .join(NAME)
                .join("target/release/afkd-github");
            ...
            drive_one_issue(home.path(), "github");
        }
    }
}
...
fn the_plugin_binary_runs_on_the_real_spine_under_probe_kinds() {
    ...
    drive_one_issue(home.path(), "github_probe");
}
...
fn every_readme_conf_fence_validates() {
    ...
    match install(home.path(), &stage(stage_dir.path())) {
        Install::BuiltIn(_) => eprintln!(
            "the afkd on PATH has `github` built in: the fences validate against its vocabulary"
        ),
...
fn the_manifest_transcribes_the_in_tree_tables() {
    ...
    let afkd_settings = "crates/github/src/settings.rs";
    let afkd_lifecycle = "crates/forge/src/lifecycle.rs";
    let afkd_lib = "crates/github/src/lib.rs";
    let (settings, lifecycle, lib) = (
        read(&src.join(afkd_settings)),
        read(&src.join(afkd_lifecycle)),
        read(&src.join(afkd_lib)),
    );
    let ported_settings = "@afkd/github/src/settings.rs";
    let ported_lifecycle = "@afkd/github/src/lifecycle.rs";
    ...
    let kinds = [
        ...
        (
            1,
            "PR_REVIEW_TRIGGER_KIND",
            "PR_REVIEW_TRIGGER_DISPLAY_NAME",
            "PR",
        ),
    ];
