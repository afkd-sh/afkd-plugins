...
//!
//! The GitHub the plugin talks to is [`fake`], the stateful loopback fake the plugin's own
//! wire suite runs against — included from the plugin's tree, never copied, so the two
//! suites cannot come to disagree about what GitHub does.
//!
//! ## No hangs on failure
//!
...
#[path = "../../../../../@afkd/github/tests/common/fake.rs"]
pub mod fake;
...
/// from the `target/release/afkd-github` its `exec` names. `RUSTUP_HOME` and `CARGO_HOME`
