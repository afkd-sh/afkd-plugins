//! Speaking the plugin wire to the real `afkd-github` exec, as afkd does: one child, one
...
impl Plugin {
    ...
    pub fn spawn() -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_afkd-github"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .expect("spawn afkd-github");
