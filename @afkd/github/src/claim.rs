...
pub(crate) struct ClaimMarker {
    /// The comment's GitHub id, unique within the unit's log.
    pub(crate) id: u64,
    /// When GitHub says the comment was posted — the **order** half: an edit must never
    /// reshuffle who spoke first.
    pub(crate) posted_at: SystemTime,
    /// The last moment GitHub reports the comment as touched — the **liveness** half,
