...
pub(crate) fn fit_poll(unit: &WireUnit) -> Option<String> {
    ...
    let with_prefix = |n: usize| {
        ...
        cut.files[brief].text = format!(
            "{}\n\n[afkd-github: this brief was cut at {n} of {} bytes to fit afkd's 64 KiB \
             plugin line — read the whole issue with the github skill]",
            &text[..n],
            text.len()
        );
        fire_line(&cut)
...
mod tests {
    ...
    fn unit(text: &str) -> WireUnit {
        WireUnit {
            ...
            env: BTreeMap::from([("GITHUB_ISSUE_NUMBER".to_string(), "7".to_string())]),
            files: vec![
                WireFile {
    ...
    fn an_oversized_brief_is_cut_at_a_char_boundary_to_fit() {
        ...
        assert!(
            text.ends_with("read the whole issue with the github skill]"),
            "{}",
            &text[text.len() - 200..]
        );
        assert!(
