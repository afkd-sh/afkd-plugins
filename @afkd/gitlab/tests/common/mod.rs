//! Speaking the plugin wire to the real `afkd-gitlab` exec, as afkd does: one child, one
...
impl Plugin {
    ...
    pub fn spawn() -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_afkd-gitlab"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .expect("spawn afkd-gitlab");
    ...
    pub fn hello(&mut self, kind: &str, settings: Value) -> Value {
        self.call(json!({"call": "hello", "proto": 1, "kind": kind,
