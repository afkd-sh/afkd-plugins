...
//!   be `release` of a crashed run's key — and GitLab's release unassigns the bot by id.
...
struct Armed<K: Units> {
    units: K,
    /// The token's user, resolved on the first call that needs it and kept.
    me: Option<User>,
...
impl<K: Units> Armed<K> {
    ...
    fn me(&mut self, diag: &dyn Diag) -> Option<User> {
        if self.me.is_none() {
...
mod tests {
    ...
    fn an_identity_that_will_not_resolve_is_an_idle_beat_retried_next_poll() {
        ...
        assert_eq!(
            f.diag.lines(),
            ["gitlab current user: no response (mock failure)"]
        );
        f.mock.clear_failure();
    ...
    fn an_undelivered_finish_is_held_and_a_later_release_recovers_it() {
        ...
        let key = f.poll()["unit"]["key"].clone();
        assert_eq!(f.mock.assignee_ids(ItemKind::Issue, 7), vec![99, 7]);
        ...
        assert_eq!(
            f.diag.lines(),
            [
                "gitlab set state: no response (mock failure)".to_string(),
                format!(
        ...
        assert!(!f.mock.has_label(ItemKind::Issue, 7, "afkd::claimed"));
        assert_eq!(f.mock.assignee_ids(ItemKind::Issue, 7), vec![99]);
