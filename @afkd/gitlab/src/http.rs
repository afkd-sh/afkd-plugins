//! The HTTP request/response spine the GitLab client rides: the pooled agent,
...
//! error mapping.
//!
//! Ported from the parts of afkd's `afkd_forge::http` the GitLab client reaches. One
...
impl<E: HttpError> HttpClient<E> {
    ...
    pub(crate) fn request(&self, method: &str, path: &str) -> ForgeRequest {
        ...
        let req = match method {
            ...
            "PUT" => ForgeRequest::WithBody(agent.put(&url)),
...
/// Percent-encode a string for use as a single URL path segment, escaping every byte
/// outside the unreserved set (`A–Z a–z 0–9 - _ . ~`) — so a name carrying a `/` becomes
...
mod tests {
    ...
    /// Every byte outside the unreserved set is escaped — a slash, a space, a colon and
    /// each byte of a multi-byte character — and the unreserved set passes through.
    #[test]
    fn encode_segment_escapes_everything_outside_the_unreserved_set() {
        assert_eq!(encode_segment("afkd-ready_1.0~x"), "afkd-ready_1.0~x");
        assert_eq!(
            encode_segment("acme/sub.group/widgets"),
            "acme%2Fsub.group%2Fwidgets"
        );
        assert_eq!(encode_segment("needs review"), "needs%20review");
        assert_eq!(encode_segment("afkd::ready"), "afkd%3A%3Aready");
        assert_eq!(encode_segment("björn/verktyg"), "bj%C3%B6rn%2Fverktyg");
