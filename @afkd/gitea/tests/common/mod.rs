//! Speaking the plugin wire to the real `afkd-gitea` exec, as afkd does: one child, one
...
impl Plugin {
    ...
    pub fn spawn() -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_afkd-gitea"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .expect("spawn afkd-gitea");
...
impl TempDir {
    pub fn new(tag: &str) -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("afkd-gitea-{tag}-{}-{n}", std::process::id()));
