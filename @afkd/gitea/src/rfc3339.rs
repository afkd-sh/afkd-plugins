...
//! stamp Gitea sends goes through, and the writer the `comments` reply's `at` comes out of.
//!
...
mod tests {
    ...
    /// `at` with: the epoch (an absent Gitea `created_at` decodes to it), a leap day, the
