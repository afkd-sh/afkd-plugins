#!/usr/bin/env python3
# remove_label.py — remove a label from the active Trello card (ADR-0046 §A).
#
# The remove half of the label trio in the bundled `trello` skill (its read half is
# list_labels.py, its add half add_label.py): it takes one board label off the card
# this run works on, so the agent can clear a flag it earlier set. The label is
# named by its id (a positional arg) OR by --name (an exact board-label name,
# disambiguated with --color when several share a name), which this helper resolves
# to an id against the board's labels. Removing a label the card does not carry is a
# no-op success, not an error. On a failure it exits non-zero so the agent surfaces
...
def delete(base, path, query):
    """DELETE `path?query`; return (status, body-bytes)."""
    conn = connection(base)
    try:
        conn.request("DELETE", f"{path}?{query}")
        response = conn.getresponse()
        return response.status, response.read()
    finally:
        conn.close()
...
def resolve_label_id(base, board, creds, name, color):
    ...
    if not 200 <= status < 300:
        print(
            f"remove_label.py: failed to read labels for board {board}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not matches:
        ...
        print(
            f"remove_label.py: no board label named {name!r}{hint}",
            file=sys.stderr,
        )
        sys.exit(1)
    if len(matches) > 1:
        ...
        print(
            f"remove_label.py: {name!r} is ambiguous — {len(matches)} labels match "
...
def main():
    ...
    parser.add_argument("label_id", nargs="?", help="the label id to remove")
    ...
    if bool(args.label_id) == bool(args.name):
        print(
            "remove_label.py: pass exactly one of a label id or --name",
            file=sys.stderr,
        )
        sys.exit(2)
    if args.color and not args.name:
        print(
            "remove_label.py: --color is only meaningful with --name",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if missing:
        print(
            f"remove_label.py: {' and '.join(missing)} must be set in the environment",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not card:
        print(
            "remove_label.py: no card id (set TRELLO_CARD_ID or pass --card)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # --- Not on the card → no-op success, no DELETE. ---
    if label_id not in on_card:
        print(f"remove_label.py: {label_id} not on card {card}")
        sys.exit(0)

    # --- Remove the label. A non-2xx is a non-zero exit the agent surfaces; the
    #     token is never echoed. ---
    query = urllib.parse.urlencode(creds)
    status, _ = delete(base, f"/1/cards/{card}/idLabels/{label_id}", query)
    if not 200 <= status < 300:
        print(
            f"remove_label.py: failed to remove {label_id} from card {card}",
            file=sys.stderr,
        )
        sys.exit(1)

    print(f"remove_label.py: removed {label_id} from card {card}")
